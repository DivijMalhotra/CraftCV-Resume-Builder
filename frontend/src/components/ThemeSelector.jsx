...
const ThemeSelector = ({selectedTheme, setSelectedTheme, resumeData, onClose}) => {
  ...
  return (
    <div className='max-w-7xl mx-auto px-4'>
        ...
        sm:p-6 bg-white rounded-2xl border border-slate-100 shadow-sm'>

            <Tabs tabs={TAB_DATA} activeTab={tabValue} setActiveTab={setTabValue}/>
            <button className='w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-3
            bg-[#3B5BDB] text-white font-bold hover:bg-[#2F4AC0] hover:shadow-lg hover:shadow-blue-200/60 rounded-xl transition-all 
            text-sm' onClick={handleThemeSelection}>
                <Check size={16}/>Apply Changes
            </button>
        </div>
        
        <div className='grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8'>
            <div className='lg:col-span-2 bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 shadow-sm'>
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[60vh] lg:max-h-[70vh] overflow-auto p-2 custom-scrollbar'>
